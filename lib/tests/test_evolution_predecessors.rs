...
fn test_walk_predecessors_basic() -> TestResult {
    ...
    let repo1 = tx.commit("test".to_string()).block_on()?;
    ...
    let repo2 = tx.commit("test".to_string()).block_on()?;
...
fn test_walk_predecessors_concurrent_ops() -> TestResult {
    ...
    let commit5 = tx
        ...
        .write_unwrap();
    tx.repo_mut().rebase_descendants().block_on()?;
    let repo5 = tx.commit("test".to_string()).block_on()?;
...
fn test_walk_predecessors_multiple_predecessors_across_ops() -> TestResult {
    ...
    let commit3 = tx
        ...
        .write_unwrap();
    tx.repo_mut().rebase_descendants().block_on()?;
    let repo3 = tx.commit("test".to_string()).block_on()?;
...
fn test_accumulate_predecessors() -> TestResult {
    ...
    let repo_a = tx.commit("a".to_string()).block_on()?;
    ...
    tx.repo_mut().rebase_descendants().block_on()?;
    let repo_b = tx.commit("b".to_string()).block_on()?;
